...
async def revoke_zombie_token(
    token_id: str,
    current_user: Dict[str, Any] = Depends(verify_role(Role.ADMIN)),
    db: Session = Depends(get_db)
):
    """
    Selectively revokes a high-risk zombie payment token while preserving
    recurring merchant subscriptions. Requires ADMIN role.
    """
